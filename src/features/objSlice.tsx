...
export const ObjSlice = createSlice({
  ...
  reducers: {
    ...
    changeActionClick: (state, action) => {
      //действия по клику
      if (
        state.currentObjectId != null &&
        state.obj.length > state.currentObjectId
    ...
    changeObjectFliesInBackpack: (state, action) => {
    ...
    changeBecomeVisible: (state, action) => {
      //становится видимым
      if (
        state.currentObjectId != null &&
        state.obj.length > state.currentObjectId
    ...
    changeBecomeUnvisible: (state, action) => {
    ...
    changeVisibility: (state) => {
      ...
      state.obj[state.currentObjectId].visible =
        !state.obj[state.currentObjectId].visible;
    },
    upObject: (state) => {
      if (
        state.currentObjectId != null &&
        state.obj.length > state.currentObjectId + 1
      ) {
        [
          state.obj[state.currentObjectId],
          state.obj[state.currentObjectId + 1],
        ] = [
          state.obj[state.currentObjectId + 1],
          state.obj[state.currentObjectId],
        ];
        state.currentObjectId = state.currentObjectId + 1;
        for (let i = 0; i < state.obj.length; i += 1) state.obj[i].id = i;
      }
    },
    downObject: (state) => {
...
export const showCurrentId = (state: any) =>
  state.objectData.present.currentObjectId;
